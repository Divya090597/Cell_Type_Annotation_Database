...
# Input File
sc_RNAseq_dir = "single_cell_analysis/GSE174401/Brain_data" 
...
meta_dir = "GSE174401/metadata"

# Output File 2 cluster level annotatiion
cluster_annotation_dir = "GSE174401_cluster_annotation_percentage"

Brain_data = readRDS("~/data/Brain_Met_Seurat.rds")
...
scRNAseqData_scaled <- as.matrix(Brain_data[["RNA"]]$scale.data)

# run ScType
es.max <- sctype_score(scRNAseqData = scRNAseqData_scaled, scaled = TRUE, gs = gs_list$gs_positive, gs2 = gs_list$gs_negative)

# merge by cluster
cL_resutls <- do.call("rbind", lapply(unique(Brain_data@meta.data$seurat_clusters), function(cl){
  es.max.cl = sort(rowSums(es.max[ ,rownames(Brain_data@meta.data[Brain_data@meta.data$seurat_clusters==cl, ])]), decreasing = !0)
  head(data.frame(cluster = cl, type = names(es.max.cl), scores = es.max.cl, ncells = sum(Brain_data@meta.data$seurat_clusters==cl)), 10)
}))
...
Brain_data@meta.data$sctype_classification = ""
for(j in unique(sctype_scores$cluster)){
  cl_type = sctype_scores[sctype_scores$cluster==j,]; 
  Brain_data@meta.data$sctype_classification[Brain_data@meta.data$seurat_clusters == j] = as.character(cl_type$type[1])
}
...
generate_cluster_celltype_df <- function(Brain_data, cell_type_column) {
  # Generate a data frame with cluster and cell type information
  cluster_celltype_df <- data.frame(
    cluster = Brain_data$RNA_snn_res.0.3, 
    cell_type = Brain_data[[cell_type_column]]
  )
